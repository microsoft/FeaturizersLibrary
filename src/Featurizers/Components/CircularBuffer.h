...
private:
    ...
    static IteratorT begin_impl(size_t max_size, size_t start_offset, VectorT &data);

    template <typename IteratorT, typename VectorT>
    static IteratorT end_impl(size_t max_size, size_t start_offset, VectorT &data);

    template <typename IteratorT, typename VectorT>
    static std::tuple<IteratorT, IteratorT> range_impl(size_t max_size, size_t start_offset, VectorT &data, size_t n, size_t offset);
};
...
typename CircularBuffer<T>::iterator CircularBuffer<T>::begin() {
    return begin_impl<iterator>(_max_size, _start_offset, _data);
}
...
typename CircularBuffer<T>::const_iterator CircularBuffer<T>::cbegin() const {
    return begin_impl<const_iterator>(_max_size, _start_offset, _data);
}

template <class T>
typename CircularBuffer<T>::iterator CircularBuffer<T>::end() {
    return end_impl<iterator>(_max_size, _start_offset, _data);
}
...
typename std::tuple<typename CircularBuffer<T>::iterator, typename CircularBuffer<T>::iterator> CircularBuffer<T>::range(size_t n, size_t offset) {
    return range_impl<iterator>(_max_size, _start_offset, _data, n, offset);
}
...
typename std::tuple<typename CircularBuffer<T>::const_iterator, typename CircularBuffer<T>::const_iterator> CircularBuffer<T>::crange(size_t n, size_t offset) const {
    return range_impl<const_iterator>(_max_size, _start_offset, _data, n, offset);
}
...
template <typename IteratorT, typename VectorT>
// static
IteratorT CircularBuffer<T>::begin_impl(size_t max_size, size_t start_offset, VectorT &data) {
    // When _cur_size is 0 we want to return an "end" iterator.
    if (data.size() == 0) {
        return IteratorT();
    }
    return IteratorT(data.data(), max_size, data.size(), start_offset);
}

template <typename T>
template <typename IteratorT, typename VectorT>
// static
IteratorT CircularBuffer<T>::end_impl(size_t max_size, size_t start_offset, VectorT &data) {
    // End location of the iterator is equal to the start_offset if the buffer is full,
    // or equal to the _cur_size if it isnt full.
    if (data.size() == 0) {
        return IteratorT();
    }
    return IteratorT(data.data(), max_size, 0, data.size() == max_size ? start_offset : data.size());
}

template <typename T>
template <typename IteratorT, typename VectorT>
// static
std::tuple<IteratorT, IteratorT> CircularBuffer<T>::range_impl(size_t max_size, size_t start_offset, VectorT &data, size_t n, size_t offset) {
    // Since this class is used for window operations only
    // number of requested elements is never bigger than the max_size
    assert(n + offset <= max_size);

    if(data.size() == 0) {
        return std::make_tuple(IteratorT(), IteratorT());
    }
    if(offset > data.size()) {
        // if even the offset is greater than cur_size, return two end pointers
        return std::make_tuple(CircularBuffer<T>::end_impl<IteratorT>(max_size, start_offset, data), CircularBuffer<T>::end_impl<IteratorT>(max_size, start_offset, data));
    }
    else if(n + offset > data.size()) {
        // if the sum of n and offset is greater than _data.size(), requested elements are bounded by end()
        return std::make_tuple(IteratorT(data.data(), max_size, data.size() - offset, (start_offset + offset) % max_size), CircularBuffer<T>::end_impl<IteratorT>(max_size, start_offset, data));
    }
    else {
        return std::make_tuple(IteratorT(data.data(), max_size, n, (start_offset + offset) % max_size), IteratorT(data.data(), max_size, 0, (start_offset + n + offset) % max_size));
    }
}
