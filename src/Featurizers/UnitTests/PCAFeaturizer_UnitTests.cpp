...
void PCATransformerTestImpl(MatrixT const &trainingMatrix, MatrixT const &label) {
    ...
    std::vector<typename NS::InputMatrixTypeMapper<MatrixT>::OutType> outputContainer = NS::TestHelpers::TransformerEstimatorTest(PCAEstimator(pAllColumnAnnotations, 0), trainingBatches, inputContainers);

    REQUIRE(outputContainer.size() == 1);

    typename NS::InputMatrixTypeMapper<MatrixT>::OutType const &    output(outputContainer[0]);
...
template <typename T>
void PCATransformerTestColMajorStandard(void) {
    // ----------------------------------------------------------------------
...
template <typename T>
void PCATransformerTestRowMajorStandard(void) {
    // ----------------------------------------------------------------------
...
template <typename T>
void PCATransformerTestMapColMajor(void) {
    // ----------------------------------------------------------------------
...
template <typename T>
void PCATransformerTestMapRowMajor(void) {
    // ----------------------------------------------------------------------
...
TEST_CASE("Standard") {
    ...
    PCATransformerTestRowMajorStandard<float>();
    PCATransformerTestRowMajorStandard<double>();
}
...
TEST_CASE("Serialization Version Error RowMajor") {
    ...
    CHECK_THROWS_WITH(
        (NS::Featurizers::PCATransformer<NS::RowMajMatrix<float>>(in)),
        Catch::Contains("Unsupported archive version")
    );
}
